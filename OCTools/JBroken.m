...
int isAppCracked() {
    ...
    //版本过老导致的可能这个文件现在么有了？
    NSString * fileName = [NSString stringWithFormat:@"%@/%@", bundlePath, [NSString stringWithFormat:@"%@%@%@%@", @"Re",@"sour",@"ceRules.p",@"list"]];
    fileExists = [manager fileExistsAtPath:(fileName)];
	if (!fileExists) {
		return YES;
	}
